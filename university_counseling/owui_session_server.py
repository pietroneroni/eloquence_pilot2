...
MODEL_ID = os.getenv("OWUI_MODEL_ID", "university-counselor:latest")
...
def create_new_session(initial_transcript: Optional[List[Tuple[str, str]]] = None) -> CounselorSession:
    ...
    logger.info("Created new counselor session %s", sid)
    return session
...
def find_or_create_session(messages: List[ChatMessage]) -> Tuple[CounselorSession, str]:
    """
    Open WebUI's standard OpenAI-compatible request usually does not include a
    stable chat_id for custom providers. We identify sessions by matching the
    request history prefix against stored transcripts.

    Example:
    - Request transcript: [user A, assistant B, user C]
    - Prefix to match:    [user A, assistant B]
    - The last user C is the new message to process.
    """
    ...
    with SESSIONS_LOCK:
        # Exact prefix match: this is the normal path when the user continues an
        # existing Open WebUI chat.
        for session in SESSIONS.values():
            if session.transcript == prefix:
                session.last_seen_at = time.time()
                return session, "existing"

        # Brand-new chat: empty prefix.
        session = create_new_session(initial_transcript=prefix)
...
def chat_completions(request: ChatCompletionRequest, raw_request: Request):
    if request.model not in {MODEL_ID, "university-counselor", "university-counselor:latest"}:
    ...
    session, status = find_or_create_session(request.messages)
    logger.info(
        "POST /v1/chat/completions model=%s session=%s status=%s messages=%d stream=%s",
        request.model,
        session.session_id,
        status,
